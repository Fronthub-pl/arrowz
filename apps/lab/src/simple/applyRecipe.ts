 ...
 * path counts no edit, so `auto` does not carve behind it. The seed is the one
 * on screen, because the recipe does not own one.
 *
 ...
 * The draw's moves are not reported: only a real clamp raises the notice.
 */
export function applyRecipe(random: boolean): void {
  ...
  // Only when it moves: `setNumber` always replaces the view slice, which
  // reaches the element as a new view on every tick of a slider drag.
