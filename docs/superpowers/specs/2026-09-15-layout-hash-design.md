...
Revision 2: after one review round of two reviewers (engine; store, CLI, server
and lab), both "With fixes". Their probes are cited where a sentence rests on
one.
...
- `ok`, `pieces`, `maxLen` and `stuck` describe the layout **as reported by the
  ...
  moment", is strictly the run's). `fingerprint`, `boardBytes` and `svg` are
  computed by the store from what it writes.
...
**A save that omits a figure keeps the stored one.** `saveLibView` posts
`{ ok, pieces, maxLen, genMs }` and nothing else. A metric absent from the
input's `metrics` (`undefined`, not `null`) keeps the layout's stored value and
the replaced recipe's stored value; a view edit must not erase `stuck`,
...
5. write the board file, the preview (written when given, removed when not —
...
batch moves on within the existing seed limit (`--max-seeds`, default 2·N).
Stdout keeps one line per written board and the summary last; the summary names
those seeds the way it names seeds that did not close:
`batch: 3/3 boards written, 6 seeds tried, already stored: 1 2 3`. A batch that
...
- an effect on the shown file calls `layoutHash(result.board)` and keeps
  `{ file, hash }` in component state; the name is used only while that `file`
  is still `result.file`, so a late hash for a replaced board is never used;
...
- `crypto.subtle` exists only in a secure context. `localhost` and `127.0.0.1`
  ...
  reports the reason through the result slice's existing `exported(file, error)`
  line; nothing rejects unhandled;
...
`readme.test.ts` guards only the knob and rule tables, so nothing fails on its
own. Both languages change together:
...
The example hashes are real: taken from a run of the command the README shows.

`HISTORY.md` is a record of what was, and is not rewritten.

In the old lab, `renderLibrary` puts `meta.id` in a monospace column; 71
characters overflow it. `lab.html` clips the id on one line with an ellipsis.
...
- **frozen values**: all 15 cases of `fingerprints.json` gain a `layoutHash`
  ...
  written by hand, per the test's header), so the values are recorded by
...
- a re-save without `stuck`, `restarts` or `backtracks` keeps the stored ones;
...
- `--count=3 --seed=1` run twice on 10×10: the second run names seeds 1–3 as
  already stored on stderr, writes seeds 4–6 (six distinct layouts, review
  probe), prints `batch: 3/3 boards written, 6 seeds tried, already stored: 1 2 3`
  and exits 0 — Goal 4 as observed behaviour; the same second run with
  `--max-seeds=3` writes nothing and exits 1;
...
- `ExportButtons.browser.test.tsx`: the download name is the layout hash; the
  button is disabled until the hash arrives; a hash that arrives after the
  board was replaced is not used. The existing cases that click the board-file
  button ('the board file is the file on screen …') and that measure its
